<template>
  <!-- fikse dette i create
    <activity-form @activity-submitted="addActivity" ></activity-form> -->
    <div class="searchWrapper">
        ...
        <div class="checkWrapper">
            ...
            <div class="newActivityButton">
              <button class="activityBtn" v-on:click="$router.push({name: 'CreateActivity'})">
                <span class="full-text">Lag ny aktivitet</span>
                <span class="plus">
                    <link href="//maxcdn.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.min.css" rel="stylesheet">
    ...
    <h1 id="pageName">Alle aktiviteter</h1>
      ...
      <li v-for="activity in filteredList" :key="activity.id" @click="$router.push(
        ...
        <div class="activityContainer">
          ...
          <div class="description">
            ...
            <div class="dateTime">
                <p>{{ activity.startTime + "-" + activity.endTime + ", " + formattedDate(activity) }}</p>
            </div>
...
export default {
  ...
  methods: {
    notFinishedActivitiesList(){
      moment.defaultFormat = "YYYY-MM-DD HH:mm";

      return this.activities.filter(activity => {
        let nowDate = moment(moment().format(moment.defaultFormat)).toDate();
        let activityEndDate = moment(activity.date + " " + activity.endTime, moment.defaultFormat).toDate();
    ...
    formattedDate(activity){
      return moment(activity.date).format("DD-MM-YYYY");
    }
  },
  computed: {
    filteredList() {
      return this.notFinishedActivitiesList().filter(activity => {
        if (this.check.length === 0) {
          return activity.title.toLowerCase().includes(this.search.toLowerCase());
        } else {
          return activity.title.toLowerCase().includes(this.search.toLowerCase())
              && this.check.includes(activity.intensity.toLowerCase());
        }
      })
    },
    dateFormat: function(){
